...
local base = lush(function()
    return {
        ...
        Exception {fg = red, gui = "nocombine,bold,undercurl"},
        Number {fg = lightRed, gui = "nocombine"},
        Float {Number},
        Identifier {fg = lightYellow, gui = "nocombine"},
        Include {fg = blue, gui = "nocombine"},
        Keyword {fg = blue, gui = "nocombine"},
        Label {fg = red, gui = "nocombine"}, Macro {Define},
...
local function merge(base, extension)
    for k, v in pairs(base) do extension[k] = v end
    return extension
end
