...
return lush(function()
    if background == "dark" then 
        return {
            ...
            TabLineSel {Normal}, TermCursor {Cursor},
            ...
            GitSignsAddSign {fg = lightGreen, gui = "nocombine"},
            GitSignsChangeSign {fg = lightBlue, gui = "nocombine"},
            GitSignsDeleteSign {fg = lightRed, gui = "nocombine"},
            GitSignsAddNr { GitSignsAddSign },
            GitSignsChangeNr { GitSignsChangeSign },
            GitSignsDeleteNr { GitSignsDeleteSign }
        }
    else
        return {
            ...
            Constant {fg = cyan, gui = "nocombine,bold"}, -- (preferred) any constant
            String {fg = black, gui = "nocombine,italic"}, --   a string constant: "this is a string"
            Character {fg = lightGreen, gui = "nocombine"}, --  a character constant: 'c', '\n'
            Number {fg = lightRed, gui = "nocombine"}, --   a number constant: 234, 0xff
            Boolean {fg = yellow, gui = "nocombine"}, --  a boolean constant: TRUE, false
            Float {Number}, --    a floating point constant: 2.3e10
            Identifier {fg = lightYellow, gui = "nocombine"}, -- (preferred) any variable name
            Function {fg = lightBlue, gui = "nocombine,bold"}, -- function name (also: methods for classes)
            ...
            Define {fg = purple, gui = "nocombine"}, --   preprocessor #define
            Macro {Define}, --    same as Define
            PreCondit {fg = lightBlack.lighten(20), gui = "nocombine,bold"}, --  preprocessor #if, #else, #endif, etc.

            Type {fg = yellow.darken(20), gui = "nocombine,bold"}, -- (preferred) int, long, char, etc.
            StorageClass {fg = orange.lighten(20), gui = "nocombine,bold"}, -- static, register, volatile, etc.
            Structure {fg = lightCyan.darken(20), gui = "nocombine,bold"}, --  struct, union, enum, etc.
            Typedef {fg = lightGreen, gui = "nocombine,bold"}, --  A typedef
            Special {fg = lightPurple, gui = "nocombine,bold"}, -- (preferred) any special symbol
            SpecialChar {fg = purple, gui = "nocombine"}, --  special character in a constant
            Tag {fg = cyan, gui = "nocombine,bold,underline"}, --    you can use CTRL-] on this
            Delimiter {fg = lightGreen, gui = "nocombine,bold"}, --  character that needs attention
            SpecialComment {fg = lightGreen.lighten(20).desaturate(40), gui = "nocombine,italic"}, -- special things inside a comment
            Debug {fg = orange, gui = "nocombine,bold"}, --    debugging statements
            Underlined {gui = "nocombine,underline"}, -- (preferred) text that stands out, HTML links
            Bold {gui = "nocombine,bold"}, Italic {gui = "nocombine,italic"},
